...
% of a zonotope according to Lemma 1 in [1]
    ...
    if sum(Qnonempty) <= 1
        Zquad = zonotopempt([c,sum(abs(G),2)]);
    else
        Zquad = zonotopempt([c,nonzeroFilter(G)]);
    end
...
% of two zonotope objects.
    ...
    % generate new zonotope
    if sum(Qnonempty) <= 1
        Zquad = zonotopempt([Z(:,1),sum(abs(Z(:,2:end)),2)]);
    else
        Zquad = zonotopempt([Z(:,1),nonzeroFilter(Z(:,2:end))]);
    end
