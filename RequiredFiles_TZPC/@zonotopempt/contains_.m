...
% ------------------------------ BEGIN CODE -------------------------------
    ...
    if isnumeric(S)
        ...
        if size(Z.Z(:,2:end),2) >= size(Z.Z(:,2:end),1)
            numberOfFacets = 2*nchoosek(size(Z.Z(:,2:end), 2), size(Z.Z(:,2:end),1)-1);
        else
            % If the zonotope is degenerate, we need to replace the dim by
            % the rank
            numberOfFacets = 2*nchoosek(size(Z.Z(:,2:end),2),rank(Z)-1);
        end
        % We can now estimate the approximate runtime of the halfspace
        % method:
        runtime_halfspaceMethod = numberOfFacets * size(Z.Z(:,2:end),1)^4;
        ...
        runtime_linprogMethod = (size(Z.Z(:,2:end), 2)+1)^(3.5) * size(S,2);
    ...
    % capsule/ellipsoid in zonotope containment
        ...
        % Now, either S is a zonotopempt, and we can use the different
        ...
        if ~isa(S, 'zonotopempt')
            ...
            if isempty(Z1.Z(:,2:end))
                res = contains_(Z2,Z1.Z(:,1),'exact',tol);
                return
            end
...
G = Z1.Z(:,2:end);
norm_Z2_nu = @(nu) zonotopeNorm(Z2, G*nu + Z1.Z(:,1)-Z2.Z(:,1));
...
% Note that we need to use nu' here instead of nu, since for surrogateopt
% the points are given as 1xn-arrays, unlike fmincon for example.
Problem.f = @(nu) -zonotopeNorm(Z2, G*nu + Z1.Z(:,1)-Z2.Z(:,1));
...
% extract data
c_s = Z1.Z(:,1); % s: small, assuming that Z1 is contained in Z2
c_l = Z2.Z(:,1); % l: large, assuming that Z1 is contained in Z2
G_s = Z1.Z(:,2:end); % s: small, assuming that Z1 is contained in Z2
G_l = Z2.Z(:,2:end); % l: large, assuming that Z1 is contained in Z2
