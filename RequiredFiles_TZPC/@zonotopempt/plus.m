...
if strcmp('zonotopempt',class(summand1))
    %initialize resulting zonotope
    Z=summand1;
    %initialize other summand
    summand=summand2;
%Is summand2 a zonotope?    
elseif strcmp('zonotopempt',class(summand2))
    %initialize resulting zonotope
    Z=summand2;
    %initialize other summand
    summand=summand1;  
...
if strcmp('zonotopempt',class(summand))
    %Calculate minkowski sum
       newzono = zonotopempt([Z.Z(:,1)+summand.Z(:,1),[Z.Z(:,2:end), summand.Z(:,2:end)]]);
%     newzono.Z(:,1)=Z.Z(:,1)+summand.Z(:,1);
%     newzono.Z(:,(end+1):(end+length(summand.Z(1,2:end)))) = summand.Z(:,2:end);
