...
methods 
    %class constructor
    function obj = intervalmpt(leftLimit,rightLimit)
        ...
        %one input
        elseif nargin==1
            if isa(leftLimit,'intervalmpt')
                obj = leftLimit;
            else%if isnumeric(leftLimit)
                obj.inf = leftLimit;
                obj.sup = leftLimit;
...
%         obj = class(obj, 'interval');
    ...
    Z = zonotopempt(obj) %Converts an interval object into a zonotope object
